...
export default class CampaignIndexViewComponent extends Component {
    ...
    getUpdatedTableData(searchText) {
        ...
        data = data.filter((campaign) => {
            if (searchText === "") {
                return campaign;
            } else if (campaign.name.toLowerCase().includes(searchText.toLowerCase())) {
                return campaign;
            }
        });
        data.map((campaign) => {
            campaign.flight = `${campaign.flight_start_date.replaceAll('-', '/')} - ${campaign.flight_end_date.replaceAll('-', '/')}`;
        ...
        return (
            <div className='budget-used-cell'>
                ...
                <div className='budget-used-bar'>
                    <div className='budget-used-bar-inside'
                         style={{
                             background: color,
                             width: `${row}px`
                         }}
            ...
            <div>
                <div className="row">
                    <div className="col-4 grid-item">
                        <input className="form-control" type="text" placeholder="Search campaigns"
